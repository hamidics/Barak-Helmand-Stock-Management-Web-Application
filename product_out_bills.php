...
divid="product_buyprozediv";
	...
	if (window.XMLHttpRequest)
		...
		<form method="post" accept-charset="utf-8" id="myform"  class="formular" action="product_out_bills.php?viewid=<?php echo $rw['id'];?>">
			<fieldset>
			
<legend align="right"><span >کد بل: <?php echo $rw['code'];?>  | تاریخ ایجاد: <?php echo $rw['created_at'];?>  <a style='color:blue; font-size:13px' href='product_out.php#tabs-4'>--بازگشت به صفحه قبل> </a></span></legend>
				
				<div id="tabledata" class="section">
				<?php
				$check=1;
					if($rw['type']=="outporze" and $rw['type']=="outccyclet" and $rw['status']!="accepted"){
				?>
				...
				<?php
				$sql="select  id, product_id from stock_transactions where bill_id='$rw[id]' and type='onsell' ";
				$rs=mysql_query($sql) or die (mysql_error());
				$i=1;
				$check=1;
				while($rwcheck=mysql_fetch_array($rs)){
						...
						if($balance==0 || $balance=="" || $balance<$r3['SUM(quantity)']){
							$check=0;
							$sq="select code, code_cu, name from products where id='$rwcheck[product_id]'";
				...
				<?php
					...
					if($check==0 and $rw['status']!="accepted" and ($rw['type']=="outporze" ||  $rw['type']=="outccyclet")){
				...
				<input type='submit'width='100px' name='accept' id='accept'  class='submit' style='border-top-left-radius: 10px ;border-top-right-radius: 10px ; border-bottom-left-radius: 10px ;border-bottom-right-radius: 10px ; cursor:pointer;color:white;   background-color:#5e6062; border:0;height:30px; width:130px;'  onmouseover='this.style.background=\"#252527\"' onmouseout='this.style.background=\"#5e6062\"' value='اجرای بل'/>";
					}
				
				?>
				
				<a href="product_out_print.php?bill_id=<?php echo $rw['id']."&type=".$rw['type']."&code=".$rw['code']."&created_at=".$rw['created_at'];?>"><img src='img/print.png'  style='margin-bottom:-4px;margin-right:10px' /></a>
				</div>
				<br/>
				...
				<?php
				$i=1;
				if($rw['type']=="outcyclet" ){
				$sql="select id, engine, shasy from production where bill_out_id='$rw[id]'";
				}else{
				$sql="Select id, quantity, product_id, description from stock_transactions  where bill_id='$rw[id]' and (parent_id=0 || parent_id='')";
				}
				$res=mysql_query($sql);
				while($row=mysql_fetch_array($res)){
				$description="";
				if($rw['type']=="outcyclet" ){
				$sq="select product_id, description from stock_transactions where engine_shasyid='$row[id]' and type='onsell'";
				...
				echo "<td><a href='product_out_bills.php?delete= $row[id]&viewid=$rw[id]&pid=$rwp[id]' class='confirm'><img src='img/cancel.png'/></a>";
				...
				<?php
					if(isset ($_GET['delete'])){
						if($rw['type']=="outporze" || $rw['type']=="outccyclet"){
							...
							mysql_query($sql) or die (mysql_error());
							$sql="update  production set status='in', bill_out_id='0' where id='$_GET[delete]' and bill_out_id='$rw[id]'";
							mysql_query($sql) or die (mysql_error());
						}
					header("location:product_out_bills.php?viewid=$rw[id]");
					die();
					}
					if(isset ($_POST['accept'])){
						$time=pdate("H:m A");
						if($rw['type']=="outporze" || $rw['type']=="outccyclet"){
							$sql="Update  stock_transactions set type='out', created_at='$_POST[created_at]', time='$time' where bill_id='$rw[id]'";
							mysql_query($sql) or die (mysql_error());
							$sql="Update  bills set status='accepted' where id='$rw[id]' ";
							mysql_query($sql) or die (mysql_error());
						}else if($rw['type']=="outcyclet" ){
							$sql="Update stock_transactions set type='out', created_at='$_POST[created_at]', time='$time' where engine_shasyid!='' and bill_id='$rw[id]' and (parent_id='' or parent_id='0') ";
							mysql_query($sql) or die (mysql_error());
							
							$sql="Update  production set status='sold' where bill_out_id='$rw[id]' ";
							mysql_query($sql) or die (mysql_error());
							$sql="Update  bills set status='accepted' where id='$rw[id]' ";
							mysql_query($sql) or die (mysql_error());
						}
