...
<?php
	...
	$billtype="";
	if($_GET['type']=="outporze"){
		$billtype="بل خروج پرزه جات";
	}else if($_GET['type']=="outccyclet") {
		$billtype="بل خروج موتور کلی";
	}else {
		$billtype="بل خروج موتور";
	}
	...
	//Main Part
				...
				$i=1;
				if($_GET['type']=="outcyclet"){
				$sql="select id, engine, shasy from production where bill_out_id='$_GET[bill_id]'";
				...
				$sq="select product_id, description, quantity from stock_transactions where engine_shasyid='$row[id]' and type='onsell' and (parent_id='0' || parent_id='') ";
				...
				if($rwp['type']==1){
					...
					$objPHPExcel->getActiveSheet()->setCellValue('F'.$j, $rwp['code'].$rwp['code_cu']."-".$rwp['name'].$type);
	...
	header('Content-Disposition: attachment;filename="product_out.xls"');
